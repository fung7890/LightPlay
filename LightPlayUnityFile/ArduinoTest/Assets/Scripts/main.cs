...
public class main : MonoBehaviour
{
    ...
    private GameObject cam, preGameObj, player, notifColor, notifColor2, notifColor3, notifColor4, contButton, instructionTxt, onScreenNotif;
    private MeshRenderer s1, s2, s3, s4, s5, s6, s7, s8, s9, s10, s11, s12, s13, s14, s15, s16;
    private Transform midOfPreGame;
    System.Random rnd = new System.Random();
    int randNum, nothing, colorChoice;
    float currTimerValue, noticeabilityTimeValue;
    public bool secondaryTaskDone, recordNoticeability;
    ...
    List<MeshRenderer> allBoxes = new List<MeshRenderer>();
    // old static order 
    ...
    // ordering
    int[] order1 = new int[300];
    int[] order2 = new int[300];
    int[] order3 = new int[300];

    int[] tmpOrder;
    List<int> conditionsRan = new List<int>(); // conditions already ran
    int currTarget, hitsTillIndicator, randColorChoice;
    ...
    void Awake()
    {
        ...
        onScreenNotif = GameObject.FindWithTag("onScreenNotif");

        s1 = GameObject.Find("s1").GetComponent<MeshRenderer>();
        s2 = GameObject.Find("s2").GetComponent<MeshRenderer>();
        s3 = GameObject.Find("s3").GetComponent<MeshRenderer>();
        s4 = GameObject.Find("s4").GetComponent<MeshRenderer>();
        s5 = GameObject.Find("s5").GetComponent<MeshRenderer>();
        s6 = GameObject.Find("s6").GetComponent<MeshRenderer>();
        s7 = GameObject.Find("s7").GetComponent<MeshRenderer>();
        s8 = GameObject.Find("s8").GetComponent<MeshRenderer>();
        s9 = GameObject.Find("s9").GetComponent<MeshRenderer>();
        s10 = GameObject.Find("s10").GetComponent<MeshRenderer>();
        s11 = GameObject.Find("s11").GetComponent<MeshRenderer>();
        s12 = GameObject.Find("s12").GetComponent<MeshRenderer>();
        s13 = GameObject.Find("s13").GetComponent<MeshRenderer>();
        s14 = GameObject.Find("s14").GetComponent<MeshRenderer>();
        s15 = GameObject.Find("s15").GetComponent<MeshRenderer>();
        s16 = GameObject.Find("s16").GetComponent<MeshRenderer>();
        colorChoice = -1;
    ...
    void Start()
    {
        ...
        notifColor2 = GameObject.FindWithTag("notifColor2");
        notifColor3 = GameObject.FindWithTag("notifColor3");
        notifColor4 = GameObject.FindWithTag("notifColor4");

        notifColor.SetActive(false);
        notifColor2.SetActive(false);
        notifColor3.SetActive(false);
        notifColor4.SetActive(false);
    ...
    IEnumerator run()
    {
        ...
        for (int x = 0; x < 2; x++)
        {
            ...
            // int condition = rnd.Next(1, 3); < -- BRING BACK
            int condition = 1; // set to 1 for testing


            // conditions: 1 - onscreen, 2 - ambient, 3 - LED 
            while (conditionsRan.Contains(condition))
            {
                condition = rnd.Next(1, 3);
            }
            ...
            if (condition == 1)
            {
                displayText(@"Click the target box amongst the boxes around you, the target box will be red. 
							Once you see an on screen indicator click according to what colour the indicator is:
							red - left arrow key
							green - up arrow key
            ...
            {
                displayText(@"
							Click the target box amongst the boxes around you, the target box will be red. 
							Once you see an ambient light indicator click according to what colour the indicator is:
							red - left arrow key
							green - up arrow key
							blue - right arrow key");
            }
...
Second Task: Click on the box when the ambient light indicator is in front you (top of the monitor). You won't progress until you find the right box so just keep trying!");
            ...
            // START PRIMARY TASK, RUN RANDOMLY CONTINUOUSLY 



            // ACTUAL TASK, RUN IT 25 TIMES ON A RANDOM INTERVAL 
            ...
            { // each iteration 
                ...
                hitsTillIndicator = rnd.Next(2, 6); // random number of box hits until indicator comes up 
                // RANDOMIZE TMPORDER HERE
                ...
                // SELECT RANDOM CUBE TO BE RED
                // CONTINUE TO APPEND RANDOM CUBE NUMBER TO TMPORDER UNTIL TIME FOR INDICATOR 
                for (int hit = 0; hit < hitsTillIndicator; hit++)
                {
                    allBoxes[tmpOrder[currTarget] - 1].material = mat2; // selected target
                    // start secondary task
                    secondaryTaskDone = false;
                    yield return new WaitUntil(() => secondaryTaskDone == true);
                    allBoxes[tmpOrder[currTarget] - 1].material = mat1; // change back after selected
                    currTarget += 1; // move on to next target
                }

                randColorChoice = rnd.Next(0, 3);

                if (condition == 1)
                {
                    if (randColorChoice == 0)
                    {
                        notifColor.GetComponent<Image>().color = Color.red;
                        notifColor2.GetComponent<Image>().color = Color.red;
                        notifColor3.GetComponent<Image>().color = Color.red;
                        notifColor4.GetComponent<Image>().color = Color.red;
                    }
                    else if (randColorChoice == 1)
                    {
                        notifColor.GetComponent<Image>().color = Color.green;
                        notifColor2.GetComponent<Image>().color = Color.green;
                        notifColor3.GetComponent<Image>().color = Color.green;
                        notifColor4.GetComponent<Image>().color = Color.green;
                    }
                    else if (randColorChoice == 2)
                    {
                        notifColor.GetComponent<Image>().color = Color.blue;
                        notifColor2.GetComponent<Image>().color = Color.blue;
                        notifColor3.GetComponent<Image>().color = Color.blue;
                        notifColor4.GetComponent<Image>().color = Color.blue;
                    }

                    notifColor.SetActive(true);
                    notifColor2.SetActive(true);
                    notifColor3.SetActive(true);
                    notifColor4.SetActive(true);

                    // start timer, record 
                    recordNoticeability = true;
                    yield return new WaitUntil(() => recordNoticeability == false);

                    notifColor.SetActive(false);
                    notifColor2.SetActive(false);
                    notifColor3.SetActive(false);
                    notifColor4.SetActive(false);
                    // once noticed, save time
                    print(noticeabilityTimeValue);
                ...
                else if (condition == 2)
                {
                    ...
                    if (randColorChoice == 0)
                    {
                        player.GetComponent<soundDetect>().allLightsOn(0);
                    }
                    else if (randColorChoice == 1)
                    {
                        player.GetComponent<soundDetect>().allLightsOn(1);
                    }
                    else if (randColorChoice == 2)
                    {
                        player.GetComponent<soundDetect>().allLightsOn(2);
                    }

                    // start timer, record 
                    recordNoticeability = true;
                    yield return new WaitUntil(() => recordNoticeability == false);
                    player.GetComponent<soundDetect>().allLightsOff();
                }
                ...
                // tmpOrder[currTarget]
                {
                    //  audio only
                    // if (condition == 1)
                    // { // CHANGE FROM TRIGDICT TO ORDER1DICT
                ...
                // player.GetComponent<soundDetect>().resetLight(); < --- BRING THIS BACK
                //     player.GetComponent<soundDetect>().onLEDActivate();
    ...
    void Update()
    {
        ...
        if ((Input.GetKeyDown(KeyCode.LeftArrow) && recordNoticeability && randColorChoice == 0) ||
            (Input.GetKeyDown(KeyCode.UpArrow) && recordNoticeability && randColorChoice == 1) ||
            (Input.GetKeyDown(KeyCode.RightArrow) && recordNoticeability && randColorChoice == 2))
        {
        ...
        else if (((Input.GetKeyDown(KeyCode.LeftArrow)) || (Input.GetKeyDown(KeyCode.LeftArrow)) || (Input.GetKeyDown(KeyCode.LeftArrow))) && recordNoticeability)
        {
            noticeabilityTimes.Add(-1);
        }
    }
}
